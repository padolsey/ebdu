    ...
    constructor() {
        this.extractionMethods = [
            ...
            async (buffer) => {
                ...
                const { data } = await Tesseract.recognize(screenshotBuffer);
                return data.text;
            },
        ];
    }
    async parsePdf(buffer) {
        for (const method of this.extractionMethods) {
            try {
                ...
                console.warn('A PDF parsing method failed. Trying the next method...', String(error).slice(0, 500) + ' [... truncated ...]');
            }
        }
