...
class UIController {
    ...
    async handleFormSubmission(req, res) {
        ...
        const documentUrlsArray = documentUrls.split(',').map((url) => url.trim());
        const tokensNumber = tokens ? parseInt(tokens, 10) : 1000;
        const nNumber = n ? parseInt(n, 10) : 3;
        try {
            const result = await embeddingsService.handleSearch(documentUrlsArray, tokensNumber, nNumber, query);
